...
function mergeObjects(objects) {
  ...
  let keysAndValues = [];
  objects.forEach((obj) => {
    keysAndValues = keysAndValues.concat(Object.entries(obj));
  });
  keysAndValues.forEach((arr) => {
    if (result[arr[0]] === undefined) result[arr[0]] = 0;
    result[arr[0]] += arr[1];
  });
  return result;
}
...
function compareObjects(obj1, obj2) {
  ...
  for (let index = 0; index < obj1Keys.length; index += 1) {
    if (obj1[obj1Keys[index]] !== obj2[obj1Keys[index]]) return false;
  }
  return true;
}
...
function makeWord(lettersObject) {
  ...
  const length = indexes.flat(1).sort()[indexes.length - 1];
...
function fromJSON(proto, json) {
  ...
  return Object.assign(Object.create(proto), objJSON);
}
